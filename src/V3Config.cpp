...
public:
    ...
    void apply(AstVar* varp) {
        for (const_iterator it = begin(); it != end(); ++it) {
            ...
            varp->addAttrsp(newp);
            if (it->type == AstAttrType::VAR_PUBLIC_FLAT_RW && it->sentreep) {
...
class V3ConfigFTask {
    V3ConfigVarResolver m_vars;
    bool m_isolate, m_noinline, m_public;
public:
    V3ConfigFTask() : m_isolate(false), m_noinline(false), m_public(false) {}
...
class V3ConfigModule {
    ...
    bool m_public;
public:
    V3ConfigModule() : m_inline(false), m_inlineValue(false), m_public(false) {}
...
typedef enum {
    ...
    PARALLEL_CASE,
    ENUM_SIZE
} V3ConfigLineAttributesType;

// Multiple attributes can be attached to a line
typedef std::bitset<V3ConfigLineAttributesType::ENUM_SIZE> V3ConfigLineAttribute;
...
class V3ConfigFile {
    ...
    bool lineMatch(int lineno, V3ConfigLineAttributesType type) {
...
public:
    ...
    void update(const V3ConfigFile &file) {
        ...
        // Copy in all ignores
        for (IgnLines::const_iterator it = file.m_ignLines.begin();
             it != file.m_ignLines.end(); ++it) {
            m_ignLines.insert(*it);
        }
        // Update the iterator after the list has changed
        m_lastIgnore.it = m_ignLines.begin();
    }
    void addLineAttribute(int lineno, V3ConfigLineAttributesType attr) {
...
class V3ConfigResolver {
    ...
    ~V3ConfigResolver() {}
public:
    inline static V3ConfigResolver& singleton() { return s_singleton; }
...
void V3Config::addCaseFull(const string& file, int lineno) {
    V3ConfigResolver::singleton().files().at(file).addLineAttribute(lineno, FULL_CASE);
}

void V3Config::addCaseParallel(const string& file, int lineno) {
    V3ConfigResolver::singleton().files().at(file).addLineAttribute(lineno, PARALLEL_CASE);
}

void V3Config::addCoverageBlockOff(const string& file, int lineno) {
    V3ConfigResolver::singleton().files().at(file).addLineAttribute(lineno, COVERAGE_BLOCK_OFF);
}

void V3Config::addCoverageBlockOff(const string& module, const string& blockname) {
    V3ConfigResolver::singleton().modules().at(module).addCoverageBlockOff(blockname);
}
...
void V3Config::addIgnore(V3ErrorCode code, bool on, const string& filename, int min, int max) {
    if (filename=="*") {
        ...
        V3ConfigResolver::singleton().files().at(filename).addIgnore(code, min, on);
        if (max) V3ConfigResolver::singleton().files().at(filename).addIgnore(code, max, !on);
        V3ConfigResolver::singleton().files().flush();
    }
}

void V3Config::addInline(FileLine* fl, const string& module, const string& ftask, bool on) {
    if (ftask.empty()) {
        V3ConfigResolver::singleton().modules().at(module).setInline(on);
    } else {
        if (!on) {
            fl->v3error("no_inline not supported for tasks"<<endl);
        } else {
            V3ConfigResolver::singleton().modules().at(module).ftasks().at(ftask).setNoInline(on);
        }
    }
}
    ...
    if (var.empty()) {
        if (attr == AstAttrType::VAR_ISOLATE_ASSIGNMENTS) {
            if (ftask.empty()) {
                ...
                V3ConfigResolver::singleton().modules().at(module).ftasks().at(ftask).setIsolate(true);
            }
        } else if (attr == AstAttrType::VAR_PUBLIC) {
            if (ftask.empty()) {
                // public module, this is the only exception from var here
                V3ConfigResolver::singleton().modules().at(module).setPublic(true);
            } else {
                V3ConfigResolver::singleton().modules().at(module).ftasks().at(ftask).setPublic(true);
            }
        } else {
            fl->v3error("missing -signal"<<endl);
        }
    } else {
        V3ConfigModule& mod = V3ConfigResolver::singleton().modules().at(module);
        if (ftask.empty()) {
...
void V3Config::applyCase(AstCase* nodep) {
    ...
    V3ConfigFile* file = V3ConfigResolver::singleton().files().resolve(filename);
...
void V3Config::applyCoverageBlock(AstNodeModule* modulep, AstBegin* nodep) {
    ...
    V3ConfigModule* module = V3ConfigResolver::singleton().modules().resolve(modname);
...
void V3Config::applyIgnores(FileLine* filelinep) {
    ...
    V3ConfigFile* file = V3ConfigResolver::singleton().files().resolve(filename);
    if (file) file->applyIgnores(filelinep);
}
...
void V3Config::applyFTask(AstNodeModule* modulep, AstNodeFTask* ftaskp) {
    const string& modname = modulep->name();
    V3ConfigModule* modp = V3ConfigResolver::singleton().modules().resolve(modname);
    if (!modp) return;
...
void V3Config::applyVarAttr(AstNodeModule* modulep, AstNodeFTask* ftaskp,
                            AstVar* varp) {
    V3ConfigVar* vp;
    V3ConfigModule* modp = V3ConfigResolver::singleton().modules().resolve(modulep->name());
    if (!modp) return;
    if (ftaskp) {
        V3ConfigFTask* ftp = modp->ftasks().resolve(ftaskp->name());
        if (!ftp) return;
